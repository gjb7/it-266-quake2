...
void weapon_potion_fire (edict_t *ent, int potion_type)
{
	vec3_t	offset;
	vec3_t	forward, right;
	vec3_t	start;
	...
	timer = ent->client->grenade_time - level.time;
	speed = GRENADE_MAXSPEED;// + (GRENADE_TIMER - timer) * ((GRENADE_MAXSPEED - GRENADE_MINSPEED) / GRENADE_TIMER);
	fire_potion(ent, start, forward, speed, radius, potion_type);
//	fire_grenade2 (ent, start, forward, damage, speed, timer, radius, potion_type);
...
void Weapon_Potion (edict_t *ent, int potion_type)
{
	if ((ent->client->newweapon) && (ent->client->weaponstate == WEAPON_READY))
	{
		ChangeWeapon (ent);
	...
	if (ent->client->weaponstate == WEAPON_FIRING)
	{
		...
		{
			ent->client->weapon_sound = 0;
			weapon_potion_fire (ent, potion_type);
		}
